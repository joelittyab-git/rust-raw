...
#[derive(Debug)]
pub struct  ClientSenderContainer<'s,T>{
     id:i32,
     thread_handle:&'s JoinHandle<()>,   //thread handle for the incoming request listener 
...
#[derive(Debug)]
pub struct ClientReceiverContainer<'s,T>{
     id:u64,
     thread_handle:&'s JoinHandle<()>,
     channel_tx:Option<Sender<T>>
}

impl <'s,T>ClientReceiverContainer<'s, T> {
     ...
     pub fn new(handle:&'s JoinHandle<()>, channel_sender:Sender<T>, key:u64)->Self{
